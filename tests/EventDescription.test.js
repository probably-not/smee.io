  ...
  describe("render", () => {
    it("renders the correct description", () => {
      ...
      expect(wrapper.find("p").text()).toBe(
        "There was a issues event received on Wednesday, December 13th 2017, 7:01:14 a.m.."
      );
    });
    ...
    it("renders the correct description when on one repo", () => {
      ...
      const wrapper = shallow(
        <EventDescription {...props} payload={payload} />
      );
      expect(wrapper.children().length).toBe(2);
      expect(wrapper.childAt(1).text()).toBe(
        "This event was sent by probot/probot."
      );
    });
    ...
    it("renders the correct description when on multiple repos", () => {
      ...
      expect(wrapper.children().length).toBe(2);
      expect(wrapper.childAt(1).text()).toBe(
        "This event was triggered against: probot/probotJasonEtco/pizza."
      );
    });
  });
});
