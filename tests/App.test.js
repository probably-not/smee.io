...
describe("<App />", () => {
  ...
  describe("render", () => {
    ...
    it("respects the filter", () => {
      wrapper.setState({
        log: [issuesOpened, issuesOpenedTwo],
        filter: "repository.name:probot",
      });
    ...
    it("respects the filter if it starts with body", () => {
      wrapper.setState({
        log: [issuesOpened, issuesOpenedTwo],
        filter: "body.repository.name:probot",
      });
      expect(wrapper.find("ListItem").length).toBe(1);
    });
  ...
  describe("togglePinned", () => {
    ...
    it("stores the pinnedDeliveries in localStorage", () => {
      wrapper.instance().togglePinned(123);
      expect(localStorage.setItem.mock.calls[0][0]).toBe(
        "smee:log:CHANNEL:pinned"
      );
      expect(localStorage.setItem.mock.calls[0][1]).toMatchSnapshot();

      wrapper.instance().togglePinned(123);
      expect(localStorage.setItem.mock.calls[1][0]).toBe(
        "smee:log:CHANNEL:pinned"
      );
