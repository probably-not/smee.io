...
export default class App extends Component {
  ...
  onmessage(message) {
    ...
    if (
      json[idProp] === undefined ||
      this.state.log.findIndex((l) => l[idProp] === json[idProp]) === -1
    ) {
      this.setState(
        {
        ...
        () => {
          localStorage.setItem(
            this.ref,
            JSON.stringify(this.state.log.slice(0, this.storageLimit))
          );
        }
      );
    }
  ...
  render() {
    ...
    if (filter) {
      filtered = log.filter((l) => {
        if (filter && filter.includes(":")) {
          ...
          if (!searchString.startsWith("body"))
    ...
    const allLogs = filtered
      .filter((item) => !this.isPinned(item))
      .map((item, i, arr) => {
        const id = item["x-github-delivery"] || item.timestamp;
        return (
          <ListItem
      ...
      <main>
        <div className="py-2 bg-gray-dark">
          <div className="container-md text-white p-responsive d-flex flex-items-center flex-justify-between">
            ...
            <div
              className="flex-items-right tooltipped tooltipped-w"
              aria-label={stateString + " to event stream"}
            >
          ...
          <div className="container-md py-3 p-responsive">
            <div className="mb-2">
              <div className="d-flex flex-items-end mb-2">
                <label
                  htmlFor="search"
                  className="d-flex flex-items-center f6 text-gray"
                >
                  <Octicon
                    icon={Search}
                    height={12}
                ...
                <a
                  className="f6"
                  href="https://github.com/jonschlinkert/get-value"
                  target="_blank"
            ...
            {pinnedDeliveries.length > 0 && (
              <>
                <h6 className="d-flex flex-items-center text-gray mb-1">
                  <Octicon icon={Pin} height={12} width={12} className="mr-1" />{" "}
                  Pinned
                </h6>
                <ul className="Box list-style-none pl-0 mb-2">{pinnedLogs}</ul>
              </>
            )}
            <h6 className="d-flex flex-items-center text-gray mb-1">All</h6>
            {allLogs.length === 0 ? (
              <div className="Box p-3 note text-center">
                All logs are pinned
              </div>
            ) : (
              <ul className="Box list-style-none pl-0">{allLogs}</ul>
            )}
          </div>
        ) : (
          <Blank />
