...
export default class ListItem extends Component {
  ...
  handleRedeliver() {
    return window
      .fetch(`${window.location.pathname}/redeliver`, {
        method: "POST",
    ...
    return (
      <li className={`p-3 ${last ? "" : "border-bottom"}`}>
        <div className="d-flex flex-items-center">
          ...
          <time className="f6" style={{ marginLeft: "auto" }}>
            {formatDistance(item.timestamp, new Date())} ago
          </time>
          <button
            onClick={this.handleToggleExpanded}
            className="ellipsis-expander ml-2"
          >
            <Octicon icon={KebabHorizontal} height={12} />
          </button>
        </div>
        ...
        {expanded && (
          <div className="mt-3">
            <div className="d-flex flex-justify-between flex-items-start">
              <div>
                <p>
                  <strong>Event ID:</strong> <code>{id}</code>
                </p>
                <EventDescription
                  event={event}
                  payload={payload}
